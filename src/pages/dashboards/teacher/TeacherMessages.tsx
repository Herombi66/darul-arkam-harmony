...
export default function TeacherMessages() {
  const [view, setView] = useState<View>('teachers_forum');
  ...
  const renderChatWindow = () => {
    ...
    const currentConversationName =
      view === 'dms'
        ...
        : view === 'parents_forum'
        ? mockParentsChannels.find(ch => ch.id === activeId)?.name
    ...
    <div className="flex h-screen bg-background text-foreground">
      ...
      <div className="flex flex-1">
        <div className="w-20 border-r border-border/80 bg-muted/30 p-2 flex flex-col items-center space-y-4">
          ...
          <button
            onClick={() => setView('parents_forum')}
            className={cn(
              'flex flex-col items-center justify-center p-2 rounded-lg w-full',
              view === 'parents_forum' ? 'bg-primary/10 text-primary' : 'hover:bg-muted'
            )}
          >
            <Users className="h-6 w-6" />
